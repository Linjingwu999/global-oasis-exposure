...
* ``_workbook_qa`` -- one of its two comparison targets,
  ...
  :func:`_privacy_qa`. The other target, ``data/Source_Data_CEE_v1_1_4.xlsx``, was
...
# (Source_Data_CEE_v1_1_4.xlsx, which ships but is a *product* of these inputs
    ...
    if declared_binary:
        not_estimable.append(
            _ne(
                ...
                "carries is generated from the same release inputs this validator "
                "checks, so a value that "
                "disagrees with the ledger is still caught upstream; free text is not.",
                remedy="Unpack the workbook XML and scan it, or compare it with "
                "the external Source Data QA before any release that mints a DOI.",
            )
        )
